...
function Activity({ feed, members, now }: { feed: FeedItem[]; members: Member[]; now: number }) {
  ...
  const rows = (copy: boolean) => recent.map((item, i) => <motion.div layout key={`${copy ? 'loop-' : ''}${item.id}`} className={`activity-row ${i === 0 ? 'activity-row--first' : ''}`} initial={copy ? false : { opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }} transition={{ duration: 0.35 }}><div className={`event-symbol event-symbol--${item.type}`}><ArenaIcon name={eventIcons[item.type] || 'spark'} size={21}/></div><div className="activity-copy"><div className="activity-line"><strong>{members.find(m => m.login === item.user)?.name || item.user}</strong><span>{item.message}</span></div><div className="activity-detail">{item.detail || item.repo || 'Team activity'}</div></div><div className="activity-meta"><strong>{item.type === 'branch-push' ? 'PENDING' : item.xp > 0 ? `+${item.xp}` : '—'} <span>{item.type === 'branch-push' ? 'PR' : 'XP'}</span></strong><span>{elapsed(item.time, now)}</span></div></motion.div>);
