...
export function FeedItem({ item, isNew }: Props) {
  ...
  return (
    <div className={`flex gap-3 px-4 py-2.5 min-h-[52px] ${isEpic ? 'feed-epic' : ''} ${isNew ? 'feed-flash-in' : ''}`}>
      {/* Type icon + neon bar */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-1.5">
          <span className={`text-[13px] font-medium truncate ${isEpic ? 'text-gold' : 'text-t1'}`}>
      ...
      <div className="flex flex-col items-end shrink-0 gap-0.5">
        {item.xp > 0 && (
          <span className={`font-mono text-[11px] text-green font-medium glow-green rounded px-1 ${isNew ? 'xp-badge-glow' : ''}`}>+{item.xp}</span>
        )}
